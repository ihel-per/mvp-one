...
package com.imvp.demo.service.command.temp;
...
public class InstagramUploadVideoRequestPzh extends InstagramRequest<StatusResult> {
        ...
        try (CloseableHttpResponse response = api.getClient().execute(post)) {
            ...
            if (!firstPhaseResult.getStatus().equalsIgnoreCase("ok")) {
                throw new RuntimeException(
                    "Error happened in video upload session start: " + firstPhaseResult.getMessage());
            }
            ...
            StatusResult uploadJobResult = api
                .sendRequest(new InstagramUploadVideoJobRequest(uploadId, uploadUrl, uploadJob, videoFile));
            log.info("Upload result: " + uploadJobResult);

            if (!uploadJobResult.getStatus().equalsIgnoreCase("ok")) {
                throw new RuntimeException(
                    "Error happened in video upload submit job: " + uploadJobResult.getMessage());
            }
...
/**
    ...
    protected StatusResult configureThumbnail(String uploadId) throws Exception, IOException, ClientProtocolException {
        try (FFmpegFrameGrabber frameGrabber = new FFmpegFrameGrabber(videoFile)) {
            ...
            StatusResult configureResult = api
                .sendRequest(InstagramConfigureVideoRequestPzh.builder().uploadId(uploadId)
                    .caption(caption)
                    .duration(length)
                    .width(width)
                    .height(height)
